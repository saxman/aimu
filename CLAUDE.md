...
- **[aimu/tools/builtin.py](aimu/tools/builtin.py)**: built-in general-purpose tools, each decorated with `@tool` for direct in-process use. Pass an entire subgroup with `tools=builtin.web + [my_tool]`:
  ...
  - **`builtin.compute`**: `calculate`, `execute_python` (subprocess-backed Python REPL, opt-in; see below). `execute_python_in_process` (the explicit in-process opt-in, faster but weaker isolation) is importable directly but not in this subgroup
  ...
  - **`builtin.misc`**: `echo`
  - **`builtin.ALL_TOOLS`**: flat list of every built-in except `execute_python`. Backs the MCP server (`python -m aimu.tools.mcp`) and `make_tools()`; prefer a subgroup when scoping an agent.
