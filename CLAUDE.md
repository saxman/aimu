...
- **[aimu/models/hf_image/hf_image_client.py](aimu/models/hf_image/hf_image_client.py)**: `HuggingFaceImageClient` + `HuggingFaceImageModel` enum.
  - `HuggingFaceImageModel` catalog: `SD_1_5`, `SDXL_BASE`, `SD_3_5_MEDIUM`, `FLUX_DEV`, `FLUX_SCHNELL`, `FLUX_2_KLEIN_4B`, `FLUX_2_KLEIN_9B`. Each member's value is a `HuggingFaceImageSpec`. The two Klein models use `Flux2KleinPipeline` (diffusers 0.37+), which is a unified pipeline: the same class handles txt2img (`image=None`) and img2img (`image=<PIL>`). Klein sets `img2img_uses_strength=False` because it conditions on the reference image directly rather than blending via a noise `strength` scalar.
