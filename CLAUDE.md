...
- **[aimu/events.py](aimu/events.py)**: the *telemetry* channel, next to `StreamChunk`'s *content* channel. A **sink** is one callable taking one event (`EventSink = Callable[[RunEvent], None]`); attach it and you see which turns ran, what payload actually went to the provider, which tools were called with what, and what a compaction dropped. A frozen-dataclass union plus one callback, deliberately **not** a `Protocol` of named methods: adding an event must never break an existing consumer (a union grows a member and old sinks ignore it; a Protocol grows a method and every implementation is suddenly incomplete). Same reasoning that keeps conversation state a `list[dict]`.
  ...
  - **Concurrent isolation**: every emit site reads the sink through `_effective_sink(client)` (ContextVar if set, else `client.events`), not `client.events` directly, so a scoped `events=` override is per-execution-context rather than per-client: two `Agent`s sharing one `model_client` and running concurrently (e.g. every worker `Agent` in a `Parallel` built via `Parallel.from_client`) each get their own OS thread / `asyncio.Task` copy and cannot clobber each other's sink (`tests/test_workflow_parallel.py` / `tests/test_aio_workflow_parallel.py`'s concurrent-workers tests). `client.events` assigned directly (not through a scoped override) has no execution-context boundary to isolate by and remains genuinely shared, by design. One case is *not* isolated, and differs by surface: a client called from **inside** a tool dispatched under `concurrent_tool_calls=True` runs in a fresh pool thread on sync (`ThreadPoolExecutor.submit`, no context copy — the override does not reach it) but inherits the run's override on async (`asyncio.TaskGroup.create_task` always copies the current context — its events land in the run's sink, attributed to the calling agent).
