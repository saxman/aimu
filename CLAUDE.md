...
- **[aimu/agents/agent.py](aimu/agents/agent.py)**: `Agent(Runner)` for autonomous, multi-round tool execution
  ...
  - `continuation_prompt`: injected only to recover a **degenerate empty turn** — a turn
    ...
    continues by calling `chat()` with no user message, injecting nothing.
    `PROVENANCE_CONTINUATION` was removed in v0.19; the loop had already stopped producing
    it.
