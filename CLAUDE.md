...
- **[aimu/__init__.py](aimu/__init__.py)** exposes the small public surface most users start from:
  ...
  - `aimu.available_text_models(*, include_hf_cache=True)`: discovery: returns *locally available* text models as `Model` enum members (running Ollama → cached HuggingFace → reachable local OpenAI-compat servers), in provider-priority order. Download-free and cloud-free (same probes as the default resolver). `aimu.resolve_default_text_model_enum(*, include_hf_cache=True)` returns the single auto-pick (env var → first available, tool-capable preferred) as an enum member, the enum-returning twin of the internal `resolve_default_text_model()` (which returns the `"provider:model_id"` string used by `client()`). **The probes read default endpoints only**, and this is a real limitation to state rather than paper over: they run before any client exists, so a per-client endpoint override (`OllamaClient(host=...)`, `base_url=`) is invisible to them. Ollama discovery hits whatever module-level `ollama.list()` resolves to (`OLLAMA_HOST`, else `127.0.0.1:11434`) and the OpenAI-compat probes hit each provider's default `base_url`. Export `OLLAMA_HOST` to make discovery, the omitted-`model` default, and ambiguous-bare-name tiebreaking consider a remote server.
...
Model-string format: `"provider:model_id[@endpoint][;flags]"`. Provider keys (when their optional dep is installed): `ollama`, `hf`, `anthropic`, `openai`, `gemini`, `lmstudio`, `ollama-openai`, `hf-openai`, `vllm`, `llamaserver`, `sglang`, `omlx`, `llamacpp`. The id is matched against the provider's `Model` enum; colons inside the id (e.g. `qwen3.5:9b`) are preserved. `@endpoint` is accepted by the **local** providers (`_ENDPOINT_PROVIDERS`) and maps to whichever kwarg that provider's SDK spells it with: `base_url=` for the OpenAI-compatible ones, `host=` for the native `ollama` one. `endpoint_kwargs(provider, base_url)` in `model_client.py` owns that mapping and is called by both the sync and async factories, so the two cannot drift. Accepting an endpoint is a **separate** policy from accepting an uncatalogued id: `_ENDPOINT_PROVIDERS` = `_ADHOC_PROVIDERS | {"ollama"}`, since Ollama's ids are registry tags whose capabilities AIMU knows (so `ollama:unknown-tag` still raises), while the ad-hoc providers' ids are user-chosen (a GGUF filename, an oMLX directory name, an LM Studio loaded-model key).
...
- **[aimu/models/_internal/chat_state.py](aimu/models/_internal/chat_state.py)**: `_ChatStateMixin` provides the I/O-free helpers shared by both bases: `system_message` lifecycle (always-live setter that swaps the in-history system entry), `reset()`, `_append_message()` (the single append seam that stamps every message with an inert append-time `timestamp`; see "Message Timestamps"), `_append_user_turn()`, `_collect_python_tool_specs()`, the capability properties (`is_thinking_model`, etc.), tool-call recording (`_prepare_tool_calls` / `_append_assistant_tool_calls` / `_record_tool_calls`), and structured-request resolution (`_structured_request`). Both `BaseModelClient` and `AsyncBaseModelClient` inherit it. None of this is duplicated across the two bases. Generation-kwarg resolution is deliberately *not* here (it serves `generate()` as much as `chat()`): it lives in [aimu/models/_internal/generate_kwargs.py](aimu/models/_internal/generate_kwargs.py), whose `_GenerateKwargsMixin` the two bases inherit alongside this one. That module owns the tier merge (`merge_generate_kwargs`, `select_profile`), tier 1 (`DEFAULT_GENERATE_KWARGS`), the `_resolve_generate_kwargs()` entrypoint every request path calls, and the `_rewrite_generate_kwargs(kwargs)` hook a provider overrides to reshape the merged dict for its own API.
...
- **[aimu/models/base.py](aimu/models/base.py)**: Defines `BaseModelClient` abstract base class with:
  - `chat(user_message=None, generate_kwargs, use_tools=True, stream=False, images=None, include=None, tools=None, audio=None, schema=None, thinking=None)`: **one model turn** against the persistent message history; returns `str`, `Iterator[StreamChunk]`, or (when `schema=` is set) a validated dataclass/Pydantic instance. A single call issues **one** model request; if the model requests tools, they are **parsed and stored** on the assistant message (`content` + `tool_calls`) and the call returns; the client does **not** execute them (the tool-loop engine in `Agent` does; see "Tool Integration"). `user_message=None` (the continuation primitive the engine uses) runs a turn on the current messages **without appending a new user turn**: nothing is added to `self.messages` before the model call. Bare `chat("q", tools=[...])` on a tool-using turn therefore returns the tool turn (often empty prose) and leaves the tool *unexecuted*; use `Agent`/`agent.as_model_client()` for a full tool-using answer. `thinking=` (`None`/`False`/`True`/`"low"`/`"medium"`/`"high"`) is the portable reasoning-effort control; see "Thinking Models" below. Concrete clients implement `_chat()`; the public `chat()` applies the `include` stream filter and the `schema=` structured-output path (see "Structured Output").
  - `generate(prompt, generate_kwargs, stream=False, images=None, include=None, audio=None, schema=None, thinking=None)`: Single-turn stateless generation. Concrete clients implement `_generate()`; the public `generate()` is provided by the base. Accepts `images=` for one-shot vision or `audio=` for one-shot audio input (same forms as `chat`) **without** touching `self.messages`, and `schema=` for structured output. `thinking=` is the same portable reasoning-effort control as `chat()`. The base validates the respective capability flag (raises `ValueError` for non-vision / non-audio models). Passing both `images=` and `audio=` raises `ValueError`.
...
- **Supporting types in base.py**:
  - `StreamingContentType(str, Enum)`: `THINKING`, `TOOL_CALLING`, `CONTINUING`, `GENERATING`, `IMAGE_GENERATING`, `AUDIO_GENERATING`, `SPEECH_GENERATING`, `DONE`; string values (`"thinking"`, etc.). Adding a member is a **breaking** change for a consumer whose `match chunk.phase` is exhaustive, so the changelog says so under `**Change**` and not only under `**New**`.
  - `StreamChunk(NamedTuple)`: `(phase, content, agent=None, iteration=0)`. The single chunk type used everywhere: `client.chat(stream=True)`, `Agent.run(stream=True)`, `image_client.generate(stream=True)`, streaming tools, all workflow runs. `content` shape depends on phase:
    ...
    - `dict {"kind", "prompt"}` for `CONTINUING`, yielded by the **streamed** tool-loop drivers immediately before a round the loop injected rather than the model: `kind` is `PROVENANCE_CONTINUATION` (a nudge after an empty turn, tools still enabled) or `PROVENANCE_FINAL_ANSWER` (the forced wrap-up at the round cap, tools disabled), and `prompt` is the string actually sent. `chunk.iteration` alone could never carry this: a tool round advances it identically, so the counter says the round moved and never says who moved it. The same value is keyed `provenance` on the injected message in `client.messages` and `reason` in `WebChannel`'s frame; three names, one value, stated in `_boundary_chunk`'s docstring rather than unified (see "Tool Integration").
    ...
    Helpers `is_text()` / `is_tool_call()` / `is_continuing()` / `is_image_progress()` / `is_audio_progress()` / `is_speech_progress()` / `is_done()` dispatch on phase. Each is named for its *phase*, never for one of the kinds the phase can carry: `is_continuing()` is true of the forced wrap-up too, which tells the model the opposite of "keep working".
...
- **[aimu/tools/builtin.py](aimu/tools/builtin.py)**: built-in general-purpose tools, each decorated with `@tool` for direct in-process use. Pass an entire subgroup with `tools=builtin.web + [my_tool]`:
  ...
  - **`builtin.fs`**: `list_directory`, `read_file`, `write_file`, `edit_file`. **The writes are in the group**, so `tools=builtin.fs` grants the filesystem, not a view of it; narrow it with `builtin.select(...)` and confirm what you do allow with `tool_approval`
  - **`builtin.compute`**: `calculate`, `execute_python` (subprocess-backed Python REPL, opt-in; see below), `run_command` (subprocess-backed shell command, opt-in; see below). `execute_python_in_process` (the explicit in-process opt-in, faster but weaker isolation) is importable directly but not in this subgroup
  ...
  - **`builtin.misc`**: `echo`
  - **`builtin.unscoped`**: `execute_python`, `run_command`, `write_file`, `edit_file` -- the one group organized by *reach* rather than subject, and exactly what `ALL_TOOLS` omits. These four let the model name a target anywhere the running account can reach (arbitrary code, an arbitrary command, an arbitrary path); every other built-in reads, computes, or writes somewhere AIMU chose (`generate_image` saves under `paths.output`), which is why the line is drawn here and not at "has a side effect". There is deliberately **no `SAFE_TOOLS` / `ALL_SAFE_TOOLS` complement**: safety is a property of a deployment, not of a tool (`get_web_content` will fetch an internal endpoint if the model names one, `web_search` hands the query to the configured SearXNG instance, `generate_image` spends money), so naming the remainder "safe" would promise what no list can deliver. Pinned by `tests/test_tools.py::test_unscoped_group_is_exactly_what_all_tools_leaves_out` and `::test_there_is_no_safe_tools_alias`.
  - **`builtin.select(tools, include=None, exclude=None)`**: narrow a tool list by name or by tool; a group may be passed whole (`exclude=builtin.unscoped`), which is how a deny-the-group-then-allow-one-member scope reads (`select(ALL_TOOLS, exclude=builtin.fs) + [read_file]`). It exists rather than a list comprehension for the `ValueError`: a comprehension filtering on a misspelled name silently keeps the tool you meant to remove, and that shows up only as an agent doing what you thought you forbade. **Only string selectors are validated**, since only they can be misspelled -- a tool passed by reference already raised at the call site, and a group legitimately names tools the list does not hold. Not a pattern language: no globs, no precedence rules.
  - **`builtin.ALL_TOOLS`**: flat list of every built-in except the `unscoped` four. Backs the MCP server (`python -m aimu.tools.mcp`) and `make_tools()`; prefer a subgroup when scoping an agent. `make_tools(allow_file_writes=True)` adds `write_file` + `edit_file`, off by default for the same reason code execution is: the returned list is handed to an agent wholesale, and asking for "the standard tools" is not asking to have your files rewritten. `make_tools(allow_code_execution=True)` adds `execute_python` but not `run_command`: that flag names *code* execution, and widening it would hand a shell to every caller already passing it, so `builtin.compute` is the only route to `run_command`.
  ...
  - **`make_document_tools(store)`**: factory parallel to `make_memory_tools` that exposes a `DocumentStore`'s richer path API as `[save_document, read_document, edit_document, list_documents, search_documents]` `@tool`s (wrapping `write`/`read`/`edit`/`list_paths`/`search_full_text`; `read_document(path, max_lines=2000, offset=1)` windows like `read_file`, and both it and `edit_document` return a message instead of raising on a missing path or a bad match, unlike `document_mcp`'s `memory_edit`, which raises). **`edit_document` exists because `read_document` windows**: without it the only way to change one line of a paper-sized document is `read_document` + `save_document`, and saving a window back deleted everything outside it, silently, while reporting success. That is now refused rather than documented: `make_document_tools` keeps a per-factory set of **digests of the document texts it has shown the model in full**, and `save_document` will not replace an existing document whose current text is not in it. Read-before-replace, the rule Claude Code's own `Write` tool enforces. Keyed by digest rather than by path so it needs no path canonicalization (it cannot drift from the store's `..`-collapsing `_normalize`) and so it expires on its own: a document changed out-of-band since the read no longer matches, and the permission lapses exactly when it should. A whole read grants it, a **truncated read revokes it** (a model reading a window is working from a partial view, whatever it knew before, including having authored the document), a successful `save_document` grants it (the model wrote that text), and `edit_document` carries it across an edit the model itself chose. Creating a new document needs no read. Pinned by `tests/test_memory_tools.py::test_saving_back_a_windowed_read_is_refused` plus the five surrounding cases (legitimate rewrite, unread document, out-of-band change, edit-after-read, create). Two shaping decisions live in the tools rather than the store: `list_documents` appends `store.unreadable_paths()` as a note (a PDF dropped in the directory otherwise reads to the model as an empty store, and the model is the only party who can tell the user to export it), and `search_documents` excerpts each match at `SEARCH_EXCERPT_CHARS` (1500) with a pointer to `read_document`, since joining up to `n_results` whole documents made one search over a paper-sized corpus a context-window flood. The names are deliberately distinct from the `make_memory_tools` triad so one agent can carry **both** a `SemanticMemoryStore` (short facts) and a `DocumentStore` (longer documents) at once. `make_memory_tools`, `make_document_tools`, and `make_retrieval_tool` are re-exported from `aimu.aio.tools.builtin` (the async agent dispatches these sync tools via `asyncio.to_thread`).
  - **`make_subagent_tool(model, *, system_message=..., tools=None, agent_types=None, max_depth=1, max_iterations=10, concurrent_tool_calls=True, deps=None, tool_name="spawn_subagent")`**: factory returning a `spawn_subagent` `@tool` for **dynamic sub-agent spawning** — the LLM delegates an independent subtask to a *fresh* `Agent` with its own isolated context at runtime (the Claude-Code `Task` pattern; AIMU's *dynamic complement* to `OrchestratorAgent`'s static roster). Each invocation builds a fresh `ModelClient(model)` (isolated history, the `make_workers` idiom) and returns `agent.run(task)`. Two shapes chosen by `agent_types`: generic `spawn_subagent(task)`, or typed `spawn_subagent(agent_type, task)` over a registry (`name -> {"system_message", "tools"?, "model"?, "thinking"?, "generate_kwargs"?, "max_iterations"?, "compaction"?}`, the closed set `SUBAGENT_SPEC_KEYS` -- a key outside it raises rather than being ignored, since an ignored key would read exactly like an applied one) whose names are folded into the tool description; an omitted `"model"` or `"max_iterations"` falls back to what this factory was built with, while an omitted `"thinking"` or `"generate_kwargs"` inherits nothing, since there is no factory-level thinking or generation tier to fall back to. `"compaction"` also has a factory tier but is read by **membership**, not `.get()`: an absent key inherits the factory's callable and `"compaction": None` turns it off for one specialist, two cases a lookup cannot tell apart. An unknown `agent_type` is returned to the model as a tool result (self-correction), while programmer errors (`max_depth < 1`, empty/malformed `agent_types`, a spec carrying a key outside the closed set, a `"max_iterations"` that is not an int >= 1, or a non-callable `compaction`) raise `ValueError` at factory-call time. **A child that exhausts its own context is also returned, not raised** (`_subagent_overflow_result`): every provider composes `ContextOverflowError` for whoever built the client ("shorten the conversation, advertise fewer tools"), and at this one seam the reader is a *different agent* than the one whose window filled, so uncaught it reads as the parent's conversation being too long. Observed in the wild, with a delegating assistant telling its user exactly that and giving up on delegation while one child had swallowed a single oversized tool result. The provider's own sentence is kept as evidence and disclaimed; a WARNING is logged; every other child failure still propagates; and on the async surface the catch sits *outside* `_run_observed`, so an observer still hears `finished(error=...)` and a front end's spawn card is still marked failed. **Parallelism is free**: with the *parent* agent's `concurrent_tool_calls=True`, multiple spawns in one turn run concurrently via the engine's existing `ThreadPoolExecutor` (sync) / `asyncio.TaskGroup` (async). `max_depth` (default 1, counting the caller as level 1) bounds recursion by only re-injecting a decremented spawn tool when `> 1`. Non-streaming by design (a streaming tool would disable the concurrent dispatch path and interleave concurrent sub-agents). It is a plain factory tool — **not** a `Runner` subclass. Async twin `make_async_subagent_tool` in [aimu/aio/tools/builtin.py](aimu/aio/tools/builtin.py) produces an `async def` tool building `aio.Agent`s; in-process providers (HuggingFace/LlamaCpp) are wrapped per spawn via a fresh sync client (aio can't build them from an enum; the weight cache prevents reload). `compaction` (factory arg + spec key) is the half that *prevents* this rather than explaining it, and only for growth spread over many rounds: `trim_messages` cannot drop the most recent group without orphaning the call it answers, so a single tool result too large for the window stays the tool's own to cap. Tests: `tests/test_subagent_tools.py`, `tests/test_aio_subagent_tools.py`. How-to: [docs/how-to/spawn-subagents.md](docs/how-to/spawn-subagents.md); demo: `examples/news-summarizer --method spawn`.
  ...
  - `execute_python(code)`: Subprocess-backed Python REPL, opt-in. Runs `code` in a fresh child process per call (`sys.executable -c ...`, `start_new_session=True`) and captures stdout plus the last expression's value. The subprocess buys a hard timeout (10s), crash isolation, no mutation of this process's imports/globals, a memory cap (enforced on Linux; best-effort on other POSIX platforms via `RLIMIT_AS`, absent on Windows -- degrades to no cap with a one-time warning rather than failing silently), and no access to this process's environment variables (`ANTHROPIC_API_KEY` and the like are invisible to the child; only a minimal allowlist -- `PATH`, `HOME`, `LANG`, etc. -- is passed through). This is isolation, not containment. It does not confine the filesystem or network -- the child runs as the same user, so a `.env`, `~/.aws/credentials`, or any other on-disk credential is exactly as readable to the child as to this process. The child also runs with restricted builtins and an import allowlist (stops accidents, not a determined attempt: a one-line expression can reach `subprocess.Popen` through the type hierarchy). Pre-imports `math`, `statistics`, `json`, `re`, `itertools`, `functools`, `datetime`, `zoneinfo`, and `numpy`/`pandas`/`scipy`/`matplotlib` when installed. Gate with `tool_approval` for untrusted callers; reach for a container when real containment is needed. `execute_python_in_process(code)` is the explicit opt-in alternative: same restricted-builtins/import-allowlist guard, but runs in this process (shares its memory, imports, and environment variables, including any API keys in `os.environ`) -- faster, weaker isolation, not in `ALL_TOOLS` or `builtin.compute`. **Not in `ALL_TOOLS`**; opt in via `tools=builtin.compute` or `make_tools(..., allow_code_execution=True)`.
  - `run_command(command, cwd="", timeout=30)`: Subprocess-backed shell command, opt-in. Runs `command` through `/bin/sh -c` (`COMSPEC /c` on Windows) and returns its exit code with stdout and stderr labelled separately; `timeout` is seconds, clamped to 600. Shares `execute_python`'s supervisor (`_run_supervised`, extracted from that tool) rather than reimplementing it: output captured to temp files rather than pipes (so a backgrounded grandchild holding the inherited fds can't keep a finished run hostage), `start_new_session=True` with a process-group SIGKILL on timeout and on any interruption, capped reads. Three deliberate differences from `execute_python`: a nonzero exit returns the output rather than an error string (`pytest` exits 1 with the answer on stdout; `git diff --exit-code` exits 1 to mean "yes, there is a diff"); a timed-out command returns whatever it printed first; no memory cap (a 512 MB `RLIMIT_AS` breaks compilers and test suites, and enforcing one on a shell child needs `preexec_fn`, unsafe alongside threads). Same isolation-not-containment disclosure as `execute_python`, one step sharper: the child reaches on-disk credentials (`.env`, `~/.aws/credentials`) as the calling user, and `kill -9` against this process's own pid is one command away. Gate with `tool_approval` for untrusted callers; reach for a container when real containment is needed. `make_command_tool(env_passthrough=...)` is the factory: the child's environment is an allowlist (`execute_python`'s, plus `SHELL`, `TERM`, `TZ`, `USER`, `LOGNAME`), so no API key in this process reaches a command; `env_passthrough` lets a host name extra variables to admit (for `gh`, `ssh`), and `run_command` is that factory called with no arguments. **Not in `ALL_TOOLS`, and not added by `make_tools(allow_code_execution=True)`**; `builtin.compute` is the only route in. Tests: `tests/test_code_execution.py`.
  - `get_web_content(url, max_chars=20000, offset=1)`: Fetches a URL and returns Markdown, for an HTML page or a PDF. Classifies by `Content-Type` with `%PDF-` magic bytes as the tiebreaker, since a PDF served as `application/octet-stream` is common. HTML keeps its recovered `Published:` line (and reports, rather than returns an empty result, when a JavaScript-rendered page has no readable text); a PDF gets one `## Page N` heading per page, is opened with an empty password first because published reports routinely carry an owner password alone, and stops extracting past 200,000 accumulated characters so a pathological document can't burn arbitrary CPU on text a character cap would discard anyway. Any other textual body (plain text, JSON, XML, or a feed format like RSS/Atom, i.e. the bare `application/json` and `application/xml`, plus any media type ending in `+json` or `+xml` regardless of top-level type, e.g. `image/svg+xml`) comes back as-is; a response with no declared `Content-Type`, or one that is not text at all (an image, an archive), is refused, reported by media type (or its absence) and size rather than read as text. Two caps: `max_chars` on the return, whose marker names the `offset=` call that continues from where it stopped (paging re-fetches and re-converts, since the tool is stateless; the PDF path's 200,000-char extraction ceiling is a separate hard limit the prepended extraction note already reports), and a 10 MB module-level limit on the download, which refuses rather than truncates and reports the bytes actually read, not `Content-Length`, which describes the encoded body and disagrees with a decoded read count for a compressed response. Converters live in the private `aimu/tools/_documents.py`.
  - `get_webpage_html(url, offset=1)`: Fetches page and returns raw HTML markup; stateless, server-rendered HTML only. The per-call window is a fixed `_HTML_WINDOW_CHARS` (20,000) rather than a caller-raisable cap, since markup costs several times the tokens of the text it carries; `offset` is the way to the rest, and the form or link a caller is after is routinely past a real page's head and navigation. For form inspection/submission with cookies, use `make_web_tools()`
  ...
  - `write_file(path, content)`: Creates a file or replaces one whole, creating parent directories. Reports which of the two happened and how many lines were replaced, so an overwrite is legible in the transcript as an overwrite rather than as an indistinguishable "wrote N lines"
  - `edit_file(path, old_str, new_str)`: Replaces **exactly one** occurrence of `old_str`. Zero matches or more than one writes nothing and returns the count: replace-first would apply an ambiguous edit to whichever match came first, which is a silently wrong edit (`DocumentStore.edit` still has that weaker semantic, via `content.replace(old_str, new_str, 1)`). Reads strictly rather than with `errors="replace"` like `read_file`, because the decoded text is written back, so the tolerance that lets a model *read* a mostly-text file would replace every undecodable byte in the user's file
  - `read_file(path, max_lines=2000, offset=1)`: Reads a window of a local file: up to `max_lines` lines starting at the 1-indexed `offset`. The cap was 200, which is under a fifth of a typical paper, so the failure mode was a model synthesizing confidently from an introduction. The truncation notice names the window it returned, the total line count, and the exact `offset=` call that continues from where it stopped, which is the remedy that scales: a 25,000-line file is readable whole by paging, while the notice's old advice to raise `max_lines` overflows the context window on exactly the files where truncation bites. A non-positive `offset`/`max_lines`, or an `offset` past the end, is returned as a teaching string (with the file's real line count) rather than raised, so the model corrects its own next call. The window, the marker, and those complaints come from `_window` / `_window_complaint` / `_past_end`, shared with `read_document`, `get_web_content`, and `get_webpage_html` so the four markers cannot drift; `_truncate` survives only for `submit_form`'s response body, the one capped output that cannot be paged because continuing the read would mean submitting the form again
...
- **[aimu/events.py](aimu/events.py)**: the *telemetry* channel, next to `StreamChunk`'s *content* channel. A **sink** is one callable taking one event (`EventSink = Callable[[RunEvent], None]`); attach it and you see which turns ran, what payload actually went to the provider, which tools were called with what, and what a compaction dropped. A frozen-dataclass union plus one callback, deliberately **not** a `Protocol` of named methods: adding an event must never break an existing consumer (a union grows a member and old sinks ignore it; a Protocol grows a method and every implementation is suddenly incomplete). Same reasoning that keeps conversation state a `list[dict]`.
  - **Event types** (all frozen dataclasses, all carrying `agent` + `iteration`, mirroring `StreamChunk`): `RunStarted`, `RunFinished` (`result` is `None` on a streamed run and on a `schema=` run; `error` when it raised), `ModelTurnStarted` (`message_count` is what *this request* carries, counting the user turn about to be appended and the system message when this turn seeds it; `1` for a stateless `generate()`), `RequestPrepared` (the post-adaptation payload, unredacted), `ModelTurnFinished` (fires for every started turn, including a failed one, with `error` set; on a streamed turn it fires when the stream is *drained*, so `usage` is real; not emitted at all on a `schema=` turn, since that path returns before `ModelTurnStarted` fires), `ToolCalled` (every outcome the model sees: success, a raising tool, invalid arguments, and a hallucinated tool name), `ToolDenied` (a `tool_approval` refusal, before the tool runs), `ContextCompacted`.
  - **`emit(sink, event)`** logs rather than propagates a sink's exception: observation must not change what it observes. **A sink must be thread-safe**: with `concurrent_tool_calls=True` the tool events come from a `ThreadPoolExecutor` (sync) / `asyncio.TaskGroup` (async) and their order is nondeterministic. `log_events(logger)` is the one-liner sink the docs lead with.
  - **Where `events=` is accepted**: `aimu.chat()` / `aimu.client()` / `client.events = ...` (the durable, always-shared setting — see below); `Agent(events=)` + `Agent.run(events=)` (a per-run override installed as the active sink for the run's execution context via a scoped `contextvars.ContextVar` (`_ACTIVE_EVENT_SINK` in [aimu/models/_internal/chat_state.py](aimu/models/_internal/chat_state.py)), reset via a `finally`); `Chain.from_client()`, `Router.from_client()`, `Parallel.from_client()`, `PlanExecuteEvaluator.from_client()`; `OrchestratorAgent._init_orchestrator()` / `.assemble()` and the three `aimu.agents.prebuilt` orchestrators (their inner `Agent` is private, so without the seam they would be unobservable). **A new workflow factory must thread `events=` down to every `Agent` it builds** — a factory that swallows it silently makes that workflow the one thing a caller cannot observe.
  - **Concurrent isolation**: every emit site reads the sink through `_effective_sink(client)`, not `client.events` directly, so a scoped `events=` override is per-execution-context *and* per-client-family rather than per-client-object or ambient. The `_ACTIVE_EVENT_SINK` ContextVar's payload is `(sink, family)`, where `family` (`_client_family(self)`) is `self` plus every inner client it delegates state to (`ModelClient`/`AsyncModelClient`'s `_client`, `_AsyncInProcessClient`'s `_sync`, `_AgenticView`/`_AsyncAgenticView`'s `_inner_client`, `FallbackClient`/`AsyncFallbackClient`'s `clients`), since a wrapper's inherited `chat()` and its own `_chat()` run with `self` bound to two different objects (see `_client_family`'s docstring). Two effects: (1) two `Agent`s sharing one `model_client` and running concurrently (e.g. every worker `Agent` in a `Parallel` built via `Parallel.from_client`) each get their own OS thread / `asyncio.Task` copy of the ContextVar and cannot clobber each other's sink; (2) a *different* client called from inside a tool (e.g. a fresh client the tool builds for itself, as `make_subagent_tool` does) never receives an ambient run's sink, on either surface and regardless of `concurrent_tool_calls`, since it is never a member of that run's family — it falls back to its own `self.events`, so give it an explicit `events=` if it needs to report anywhere (`tests/test_workflow_parallel.py` / `tests/test_aio_workflow_parallel.py`'s concurrent-workers tests; `tests/test_events.py` / `tests/test_aio_events.py`'s concurrent-tool-dispatch tests, both the different-client and same-client cases). `client.events` assigned directly (not through a scoped override) has no execution-context boundary to isolate by and remains genuinely shared, by design. The one case that still depends on the surface is a tool that calls the *same* client the override was installed for (e.g. reusing `ctx.deps`): a concurrently dispatched sync tool runs in a fresh `ThreadPoolExecutor` thread with no context copy, so the override is invisible there even though the client matches; a concurrently dispatched async tool's `asyncio.TaskGroup.create_task` always copies the current context, so it *is* visible there, attributed to the calling agent. Sequential tool dispatch (the default) sees it on both surfaces, since no thread/task boundary is crossed.
  - **Attribution**: the client cannot know which agent or loop round it is serving, so the tool-loop engine installs a wrapper (`_attributing` in [aimu/agents/_tool_loop.py](aimu/agents/_tool_loop.py)) that stamps `agent`/`iteration` on the way past. The `schema=` structured path uses `_AgentLoopMixin._structured_sink()` / `_structured_run_events()` ([aimu/agents/_loop.py](aimu/agents/_loop.py)) for the same reason.
  - **`client.last_request`**: the payload of the most recent request, post-adaptation, on every client (cleared at the top of each `chat()`/`generate()` and by `reset()`; delegated through `ModelClient` / `_AgenticView` / `FallbackClient` and the async twins). Providers set it through the single seam `BaseModelClient._record_request(payload)`, which also emits `RequestPrepared`.
  - **`_record_request` is mandatory on every provider request path.** It is a seam rather than a line each provider writes because this codebase already lost one cross-cutting rule that way (three of five providers dropped a generate_kwargs tier when the merge was per-provider). `tests/test_request_legibility.py::test_every_client_records_its_request` fails if a shipped client's request path records nothing; the two in-process providers (HuggingFace, LlamaCpp) record the nearest equivalent, since they have no wire payload.
  - **Tests**: `tests/test_events.py`, `tests/test_aio_events.py`. Docs: [docs/how-to/observe-a-run.md](docs/how-to/observe-a-run.md), [docs/reference/api/events.md](docs/reference/api/events.md).

### Context Management

- **[aimu/context.py](aimu/context.py)**: plain functions over the conversation, the same shape as `aimu/rag/` and for the same reason: conversation state is a `list[dict]` you can print and edit, so the tools that reshape it take a list and return a new one, rather than a `ContextPolicy` class applied invisibly inside a client.
  - `count_tokens(messages, counter=None)`: **an estimate**, defaulting to `len(text) // 4` over each message JSON-serialized *minus* the inert keys (`timestamp` / `thinking` / `provenance`, which never reach a provider). Pass `counter=` for a real tokenizer. The only exact count is post-hoc, via `client.last_usage`.
  - `trim_messages(messages, max_tokens, *, keep_system=True, keep_last=2, counter=None)`: drops oldest-first over the non-system messages, **one indivisible group at a time** — an `assistant` message carrying `tool_calls` is always dropped with every `tool` message answering it. That invariant is the whole reason this exists over a `messages[-n:]` slice, which produces exactly the orphaned shape every provider rejects. `keep_last` counts **messages**, not exchanges, extended outward to a whole group when the raw count would split one. `keep_system=True` also **moves** system messages to the front (invisible for the usual single leading prompt; a mid-conversation system message is hoisted).
  - `summarize_messages(client, messages, *, keep_last=2, prompt=None)`: replaces the prefix with one `{"role": "system"}` summary produced by a single `client.generate()` call, keeping the same group-aware tail. `client` is any object with `generate(prompt) -> str`, taken as an argument so this module stays provider-free.
  - `ContextOverflowError` ([aimu/models/_base/shared.py](aimu/models/_base/shared.py), exported from `aimu`, `aimu.models`, `aimu.aio`): the portable "this conversation no longer fits" error, alongside `ModelConnectionError`.
  - **`Agent(compaction=...)` / `Agent.run(compaction=...)`**: an opt-in callable (`list[dict] -> list[dict]`) applied before **every** model turn of a run. The tool-loop engine's `_maybe_compact()` runs it, diffs before/after **by content** (not identity, so a normalize/dedupe callable that rebuilds dicts is not mistaken for a drop), and when anything was dropped rewrites `client.messages`, emits `ContextCompacted` (carrying **copies** of the dropped messages, so a later in-place write cannot mutate an already-emitted event), and logs a WARNING **even with no sink attached** — an automatic rewrite of a caller's conversation is never silent. A `compaction` callable that raises propagates: a compaction that cannot be trusted should stop the turn, not be skipped while the caller believes their context is managed. An explicit `client.messages = trim_messages(...)` warns nothing; the caller did it and holds both lists.
  - **[aimu/aio/context.py](aimu/aio/context.py)**: only `summarize_messages` gets an async twin (it is the one function that makes a model call; it imports the grouping/partition helpers from `aimu.context` rather than duplicating them). `count_tokens` / `trim_messages` are pure and synchronous, so both surfaces call `aimu.context` directly.
  - **Tests**: `tests/test_context.py`, `tests/test_aio_context.py`. Docs: [docs/how-to/manage-context.md](docs/how-to/manage-context.md).
...
- **[aimu/memory/document_store.py](aimu/memory/document_store.py)**: `DocumentStore(MemoryStore)`: path-based document store
  ...
  - `edit(path, old_str, new_str)`: Replace the **one** occurrence of `old_str`; raises `ValueError` if it is absent **or appears more than once**, writing nothing either way. It replaced the first of several until v0.31, which answered a question the caller did not ask, silently, in the file; the message names the count so there is something to aim at. Same rule as the `edit_file` built-in, and the same `ValueError` type the missing-match case already raised, so an existing `except ValueError` stays correct. For the first of several deliberately: `store.write(p, store.read(p).replace(old, new, 1))`
  ...
  - `unreadable_paths()`: paths of files in the directory that are not readable UTF-8 text (a PDF, a Word export). Excluded from `list_paths`/`search_full_text`, so this is what lets a caller distinguish "nothing here" from "what you gave me is not text"; `make_document_tools`' `list_documents` appends them to its output for exactly that reason. Rescans; `[]` for an ephemeral store; dot-files never reported
  ...
  - **With `persist_path` set, the directory *is* the store**: `read` opens the file and `list_paths` / `search_full_text` scan the directory on **every call**, so a document copied in out-of-band (by a user, or another process) is visible immediately. There is deliberately no construction-time snapshot and no `refresh()`: the store previously walked the directory once in `__init__` and treated the resulting dict as authoritative, which made a hand-placed file invisible for the life of the process with nothing logged (worse under `document_mcp`, whose store is built at *import* time). Removing the second source of truth beats a `refresh()` a host must remember to call and an agent cannot trigger. The `_docs` dict survives only as the ephemeral-mode backing store. Documents are UTF-8 text: a non-text file is skipped with a `WARNING` naming it (once per path per instance), and dot-files / dot-directories (`.DS_Store`, `.git/`) are skipped silently. For a read-only folder of human-supplied reference material, the read half of `builtin.fs` is usually the better fit (`builtin.select(builtin.fs, exclude=builtin.unscoped)`, since the group itself now carries `write_file` / `edit_file`); `DocumentStore` earns its keep when the agent writes back.
...
- **[aimu/agents/agent.py](aimu/agents/agent.py)**: `Agent(Runner)` for autonomous, multi-round tool execution
  - Constructor: `Agent(model_client, system_message=None, name=None, tools=None, max_iterations=10, continuation_prompt=..., reset_messages_on_run=False, final_answer_prompt=None, deps=None, tool_approval=None, concurrent_tool_calls=False, thinking=None)`. `system_message` is the second positional argument. `name` is optional, auto-derived from `id(agent)` as `"agent-{hex}"` when unset. Tool configuration (`tools`, `deps`, `tool_approval`, `concurrent_tool_calls`) lives on the `Agent`, not the model client.
  - Wraps any `BaseModelClient` and **composes a tool-loop engine** (`_ToolLoop` / `_AsyncToolLoop`) per run via `_make_tool_loop(...)`. `chat()` is a **single model turn** (it parses and stores any requested tool calls; it does **not** execute them). The engine is the loop over that: `chat(task, tools=...)`, then dispatch the requested tools, then `chat()` (no user message — a continuation turn), until a turn makes no tool calls, or the loop has made `max_iterations` real model calls. **`max_iterations` is the maximum number of model calls the loop itself makes** — the initial turn plus every continuation/tool-follow-up turn the bounded loop drives; it is not a round count with a separate off-the-books turn baked in. The one deliberate exception is the forced wrap-up call (see `final_answer_prompt` below): it runs *after* this cap is reached, specifically to guarantee an answer once the cap is hit, and is never counted against `max_iterations`. Sync (`aimu/agents/_tool_loop.py`) and async (`aimu/aio/_tool_loop.py`) implement this identically — before v0.22, the two async drivers (`run` and `run_streamed`) let the bounded loop run one round further than `max_iterations` permitted, so an async agent made one more real model call than a sync agent given the same `max_iterations`; both drivers were aligned down to the (already-correct) sync convention. **No synthetic continuation prompt is injected** into the transcript.
  ...
  - `continuation_prompt`: injected only to recover a **degenerate empty turn** — a turn
    that returned no content and no tool calls. Between successful tool rounds the loop
    continues by calling `chat()` with no user message, injecting nothing. The recovery
    nudge it injects on the degenerate-turn path *is* tagged `PROVENANCE_CONTINUATION`
    (see "Message Provenance" below), so a replayed transcript can still tell it apart
    from a genuine user turn.
  ...
  - `run(task, generate_kwargs, stream=False, images=None, tools=None, deps=None, tool_approval=None, schema=None, thinking=None)`: returns final response string or `Iterator[StreamChunk]`. Streamed chunks carry `agent` (the agent name) and `iteration` (loop index) fields. `tools=` / `deps=` / `tool_approval=` / `thinking=` are per-run overrides of the agent's configured fields (`None` uses the field, so `thinking=False` is a real override rather than an absent one). `schema=` short-circuits to a single structured-output turn (no tool loop). `thinking=` is the portable reasoning control (`True`/`False`/`"low"`/`"medium"`/`"high"`), forwarded as the **public argument** (not a pre-resolved request) to *every* `chat()` the run makes: each tool round, the continuation nudge, the forced wrap-up, and the `schema=` turn. Validation and the warn-once behavior therefore stay on the model client, where the model is known, and the agent makes no capability decisions. The overrides are passed to the engine (`_make_tool_loop`), which advertises the effective tools on every `chat()` call; after the run, `model_client.tools` is the transient default (`[]`) since the `Agent` supplies tools per call. Not added to the `Runner` ABC or workflow classes (they compose sub-runners and have no single tool list or model turn to apply them to).
...
- **[aimu/agents/skill_agent.py](aimu/agents/skill_agent.py)**: `SkillAgent(Agent)`: adds skill discovery and injection
  - Extends `Agent` with a `skill_manager: SkillManager` field (defaults to `SkillManager()` for auto-discovery) and a `script_env: dict[str, str] | None` field (default `None`), host context merged over the inherited environment for every skill script the agent runs. It is a field rather than a `build_skills_server(manager, env=...)` argument because the agent builds that server itself, at both build sites (`_setup_skills()` and `reload_skills()`), so a host holding only the agent had no other way to reach the scripts. Mirrored on `aio.SkillAgent`.
...
- **[aimu/agents/prebuilt/](aimu/agents/prebuilt/)**: Ready-to-use `OrchestratorAgent` subclasses demonstrating the orchestrator + worker tools pattern
  ...
  - `ResearchReportAgent`: orchestrator + `research_overview`, `find_examples`, `find_counterpoints` worker tools; accepts optional `worker_tools: list[Callable]` to give workers live tool access (e.g. `[builtin.web_search, builtin.get_web_content]`) and automatically enables `concurrent_tool_calls` on the orchestrator
...
- **Skill authoring** -- **[aimu/skills/authoring.py](aimu/skills/authoring.py)** (sync filesystem work, surface-shared) + `SkillManager.refresh()`:
  - `write_skill(name, description, body, *, skills_dir, overwrite=False, metadata=None, scripts=None) -> Path`: writes `skills_dir/<name>/SKILL.md` (YAML frontmatter + markdown body, the format `SkillManager` discovers) plus any `scripts` (a `{filename: source}` mapping, each written via `write_skill_script`). Validates that `name` is a slug (lowercase-with-hyphens, no path separators -> traversal guard) and `description` is non-empty; refuses to clobber unless `overwrite=True`; round-trips through the manager parser so a malformed authored file fails loudly at the write site. **Emits only `name`, `description`, and `metadata`**, so it is the create path and not an edit path: pointing it at an existing skill with `overwrite=True` drops the spec's optional `license` / `compatibility` / `allowed-tools` and any non-spec key. `update_skill` exists because of that.
  - `write_skill_script(name, filename, content, *, skills_dir) -> Path`: writes `skills_dir/<name>/scripts/<filename>` (`.py` or `.sh`, validated: no path separators, lowercase stem) and marks a `.sh` executable. Deliberately does **not** touch `SKILL.md`: attaching a script is not a change to the skill's prose, and routing it through `write_skill(overwrite=True)` is what used to drop the frontmatter keys above on every script write.
  - `update_skill(name, *, description=None, body=None, skills_dir) -> Path`: the edit path, and the counterpart to `write_skill`'s refusal to clobber. Changes only what is passed; reads the existing frontmatter and writes every other key back as it was (`license`, `compatibility`, `allowed-tools`, `metadata`, and anything outside the spec), which is the whole reason it is not `write_skill(overwrite=True)`. No rename (the name is the skill's address and the spec requires it to match the directory) and no `metadata` parameter (a host's provenance record; an edit path able to rewrite it would out-reach the create path, which cannot set it). Raises `FileNotFoundError` for an unknown skill and `ValueError` when neither field is given.
  ...
  - `make_skill_update_tool(manager, skills_dir) -> Callable`: returns an async `@tool update_skill(skill_name, description=None, body=None)` over the function above, then `manager.refresh()`. Needs no agent (an edit to a skill's text changes none of its tools, so nothing reloads), unlike `make_skill_script_tool`. A missing skill or a call with nothing to change returns a sentence rather than raising, since both are the model's to correct next round; a malformed file still raises. Note the module-level `_update_skill` alias: the tool's function *is* named `update_skill`, so inside the factory that name is local and the public function is otherwise unreachable.
  - **[aimu/skills/frontmatter.py](aimu/skills/frontmatter.py)**: `split_frontmatter(content) -> (frontmatter | None, body)`, `load_frontmatter(text) -> dict` (the lenient colon-quoting fallback for hand-written files; raises plain `ValueError`), `render_frontmatter(fields) -> str` (`yaml.safe_dump`, `sort_keys=False`, wide, so a description containing `": "` is quoted and a long one is not folded). The one definition of the file format, shared by discovery, `load_body`, and authoring; it imports nothing else in the package, which is why its parse error is a bare `ValueError` that `SkillManager` wraps into `SkillLoadError` with the path.
  - `make_skill_script_tool(agent, manager, skills_dir) -> Callable`: returns an async `@tool add_skill_script(skill_name, filename, content)` that writes the script via `write_skill_script`, refreshes the manager, and `await agent.reload_skills()` so the new `{skill}__{stem}` tool is callable in the *same* turn (the one authoring tool needing the agent, which is why it takes one). Re-calling with the same filename replaces the script in place, which is how a broken one is fixed; its docstring says so, because a model reaching for a fresh filename leaves the broken script registered. **Full access**: skill scripts run as real subprocesses with the user's privileges.
  ...
  - Exported from `aimu.skills` (`write_skill`, `write_skill_script`, `update_skill`, `make_skill_authoring_tool`, `make_skill_script_tool`, `make_skill_update_tool`); intentionally *not* re-exported from `aimu.aio` (surface-agnostic filesystem code that returns an async tool).
...
**Text catalogs share one facts table.** A model's intrinsic capabilities (`tools`/`thinking`/`vision`, `thinking_levels`/`thinking_optional`, and the card's `generation_kwargs`/`nonthinking_generation_kwargs`) are properties of the *weights*, not of who serves them, so they are declared exactly once, in `MODEL_FACTS` (a private table in [aimu/models/_catalog.py](aimu/models/_catalog.py)), keyed on the cross-provider enum-member name. Every **local-runtime** catalog (`OllamaModel`, `HuggingFaceModel`, `LlamaCppModel`, and the seven `*OpenAIModel` local-server catalogs) wires into that table instead of restating the flags. The three **cloud** catalogs (`AnthropicModel`, `OpenAIModel`, `GeminiModel`) are single-provider by nature, so they still declare a bare `ModelSpec(...)` directly and have no `MODEL_FACTS` entry.

When adding a new model to a local-runtime catalog:
1. **If the model is new to AIMU** (no other catalog serves it yet), add its intrinsic facts to `MODEL_FACTS` in `aimu/models/_catalog.py`: `"MODEL_NAME": ModelFacts(tools=..., thinking=..., vision=..., generation_kwargs=...)`. Skip this step if another catalog already carries the same enum-member name — the facts already exist.
2. In the provider catalog's `Model` class, add `MODEL_NAME = Wire(id)` — just the wire id the server accepts; the flags resolve from `MODEL_FACTS` at class-construction time (`Model.__init__` calls `resolve_wire()`). Only pass a keyword when this serving path can't deliver a capability the facts declare (e.g. `Wire(id, why="no mmproj projector loaded by default", vision=False)`); overriding an intrinsic flag without `why=` raises at import time. Serving-path-only flags (`structured_output`, `audio`) are declared freely, no `why=` needed — they describe the runtime, not a contradiction of the weights. For HuggingFace, the enum value is a tuple `(Wire(...), ToolCallFormat.XXX[, think_opener_in_prompt])` because HF carries provider-specific extras alongside the wire.
3. `TOOL_MODELS`, `THINKING_MODELS`, `VISION_MODELS`, and `AUDIO_MODELS` lists are derived automatically from the resolved flags; no manual update needed.
4. **Cross-provider consistency**: the `Wire.id` is the provider-specific *wire* identifier (correctly different per provider: `qwen3:8b` vs `Qwen/Qwen3-8B` vs `qwen3-8b.gguf`), but the enum-member **name** (`QWEN_3_8B`) is both the `MODEL_FACTS` key and the cross-provider identity `resolve_model_enum` searches for a bare name. Keeping the facts in one shared table makes drift between providers structurally impossible for intrinsic flags; `tests/test_model_catalog_consistency.py` still enforces that every provider serving a given name has a matching `MODEL_FACTS` entry. `structured_output` and `audio` stay per-catalog (serving-path flags: Ollama grammar-enforces JSON for any model; only some paths expose audio input).
5. Test with `pytest tests/test_models.py --client=<client> --model=<MODEL_NAME>`
6. `docs/reference/model-matrix.md` is **generated**, not hand-maintained: run `python scripts/generate_model_matrix.py --write` after any catalog change and commit the regenerated file. `tests/test_docs_model_matrix.py::test_matrix_tables_match_the_generator` fails the suite if you forget.

**Other modalities** use the same pre-facts-table pattern with their own spec type and enum: image (`HuggingFaceImageSpec`/`GeminiImageSpec` in `HuggingFaceImageModel`/`GeminiImageModel`), audio (`HuggingFaceAudioSpec` in `HuggingFaceAudioModel`), speech (`HuggingFaceSpeechSpec`/`OpenAISpeechSpec` in `HuggingFaceSpeechModel`/`OpenAISpeechModel`) -- add an enum member with a `ModelSpec`-family value directly, no `Wire`/`MODEL_FACTS` indirection (each is served by exactly one provider today, so there is nothing to keep from drifting). For a one-off custom model, construct the spec object and pass it to the factory (`ImageClient(spec)`), the explicit escape hatch, rather than a string. Test with the modality's live suite (e.g. `pytest tests/test_images.py --image-client=hf --image-model=<NAME>`).
...
**Portable generation kwargs, and one declared verdict per provider** (sync and async): AIMU accepts eight generation parameters under one portable name each -- `temperature`, `top_p`, `top_k`, `min_p`, `presence_penalty`, `repetition_penalty`, `max_tokens`, and `context_length` (`PORTABLE_GENERATE_KWARGS` in [aimu/models/_internal/generate_kwargs.py](aimu/models/_internal/generate_kwargs.py), which also owns the `CONTEXT_LENGTH_KWARG = "context_length"` constant) -- whatever the backend spells them. Both caller tiers already existed, so no signature changed: `client.default_generate_kwargs = {"context_length": 32768}` for every call on a client (tier 3), `chat(generate_kwargs={"context_length": 8192})` for one (tier 4), and a per-call `None` means *unset* on every one of the eight, which is how a call cancels a client default. Each client declares **one class attribute**, `GENERATE_KWARG_SUPPORT`: per portable key, either the backend's own spelling (a `str`, equal to the key when it passes through unchanged, so the base renames into it) or an `Unsupported(remedy)` (drop the key, and name `remedy` in the warning). `apply_kwarg_support()` runs it on the base, in `_GenerateKwargsMixin._resolve_generate_kwargs`, **after** the four-tier merge and **before** the provider's `_rewrite_generate_kwargs` hook, for the same reason the merge itself lives there: dropping a key a backend cannot take is a rule that must hold on *every* provider, and an opt-in hook cannot carry one -- a provider that forgot the drop would put an unknown parameter on the wire, or (on Ollama, whose SDK types `options` as a pydantic `Options` model that discards unknown keys on validation) lose it in silence. What stays in a hook is what a rename cannot express: where a surviving key *goes* (the local OpenAI-compatible servers read `top_k` / `min_p` / the repetition knob from `extra_body`; `AnthropicClient` routes `temperature` / `top_p` / `top_k` there too, since `anthropic` 1.x removed all three from the `messages.create()` signature), and a rename that depends on the *model* rather than the client (the o-series `max_completion_tokens`). Two rules keep the warnings honest: **only the caller's own two tiers warn** (a model card profile's unsupported key is dropped silently, since every Qwen card carries `min_p` and `repetition_penalty` and reporting on the merged dict would fire once per client for a value the user never set), and **`None` is unset, never unsupported**. Warn-and-drop rather than raise follows the same rule as `thinking=` ("validate the argument, never the model"), so moving a working client default to another provider never turns into an exception mid-run; silent drop would violate principle 6. **Only Ollama's native API sizes the context window per request** (`context_length` -> `num_ctx`, on both `OllamaClient` and `AsyncOllamaClient`); everywhere else that key is dropped and `_warn_once` fires with the provider's remedy: llama.cpp's load-time `n_ctx=`, `OLLAMA_CONTEXT_LENGTH` for `ollama-openai`, server launch flags for the other OpenAI-compat servers (`--ctx-size` / `--max-model-len`), the weights' `max_position_embeddings` for HuggingFace, and "fixed by the provider" for Anthropic / OpenAI / Gemini. A *family* declares the table once and its members inherit it, overriding single keys where they differ: the cloud endpoints (`CLOUD_OPENAI_GENERATE_KWARGS`) declare `top_k` / `min_p` / `repetition_penalty` unsupported, `LlamaServerOpenAIClient` renames the repetition knob to llama.cpp's own `repeat_penalty`, and `OllamaOpenAIClient` declares all three unsupported because Ollama's OpenAI shim maps a fixed OpenAI field set onto its native call and reads none of them. Each family's dict is defined once in the sync provider module and imported by the async twin, so the two cannot drift on a rename. The delegating wrappers (`ModelClient`, `_AgenticView`, `FallbackClient`, `_AsyncInProcessClient` and its HF/llamacpp subclasses) declare nothing: they hand a whole request to an inner client, whose declaration is the one that fires. `tests/test_generate_kwargs_merge.py` pins the renames, the drops, the once-per-client warning, the `None` cancellation, the card-tier silence, the `extra_body` routing, the end-to-end `options["num_ctx"]`, and a guard (`test_every_client_declares_a_verdict_for_every_portable_key`) that fails when a shipped client leaves any of the eight undeclared. See [docs/how-to/set-context-length.md](docs/how-to/set-context-length.md) and [docs/reference/provider-matrix.md](docs/reference/provider-matrix.md).

**Portable thinking control** (`thinking=` on `chat()`/`generate()`, sync and async): `None` (default) leaves a provider's own behavior untouched; `False` disables reasoning and switches to the model's instruct-mode sampling profile; `True` enables it at the model's own default effort; `"low"`/`"medium"`/`"high"` requests a specific effort level. One rule governs resolution: **validate the argument, never the model**. An invalid value raises `ValueError`. A model that cannot honour a valid request (no effort-level control, or a model that cannot disable reasoning) logs a deduplicated warning and continues instead, so swapping models never turns working code into an exception. The intrinsic capability lives on `ModelSpec` (`thinking_levels`, `thinking_optional`, `nonthinking_generation_kwargs`, above); each client translates a resolved request into its own mechanism. `Agent` carries the same control as a field plus a per-run `run(thinking=...)` override, threaded through the tool-loop engine to every model turn of a run (see the `Agent` entry above). `aimu/models/_internal/thinking.py` carries the shared level vocabulary, `resolve_thinking()`, and the reserved `_thinking` key that threads a resolved request through `generate_kwargs` without changing any `_chat`/`_generate` signature. Generation-kwarg precedence is a separate module, `aimu/models/_internal/generate_kwargs.py`, since it serves `generate()` as much as `chat()`: `select_profile(model, resolved)` picks the thinking or instruct profile off the card, and `merge_generate_kwargs(model, fallbacks, client_defaults, generate_kwargs)` layers the four tiers into a fresh dict (reading the reserved thinking key without popping it, since each provider still needs it downstream). Providers call neither directly, and never merge at all: the module's `_GenerateKwargsMixin` (inherited by `BaseModelClient` and `AsyncBaseModelClient` alongside `_ChatStateMixin`) carries `_resolve_generate_kwargs()`, which gathers the tiers off `self` and hands the merged dict to the provider's `_rewrite_generate_kwargs(kwargs)` hook (default: no rewrite), so a provider only ever declares its request-shape rewrites. Making the merge unskippable rather than a line every provider had to remember is deliberate: five providers each spread the tiers by hand and three of them dropped one. `tests/test_generate_kwargs_merge.py` fails if a shipped client overrides `_resolve_generate_kwargs` instead of the hook. **The four tiers, lowest precedence first**: (1) `Client.DEFAULT_GENERATE_KWARGS`, the library's own fallbacks for what nobody else sets (`max_tokens`: `CLOUD_MAX_TOKENS` = 16000 on Anthropic/OpenAI/Gemini, `LOCAL_MAX_TOKENS` = 4096 on llama.cpp, HuggingFace (as `max_new_tokens`) and the local OpenAI-compat servers; both constants live in `aimu/models/_internal/generate_kwargs.py` and are shared by import so the families and the two surfaces cannot drift. Cloud gets the vendors' non-streaming guidance since a cloud endpoint stops at EOS and bills per token; local stays lower because a local server spends wall-clock on every token it is allowed, and a quantized model that never emits EOS spends all of them. Empty on Ollama, which falls through to the server's defaults); (2) the model card (`ModelSpec.generation_kwargs`, or `nonthinking_generation_kwargs` when `thinking=False` resolved off); (3) `client.default_generate_kwargs`, the caller's standing kwargs for every call on that client instance; (4) the per-call `generate_kwargs`. Tier 1 is deliberately the **weakest**: a library-chosen `temperature=0.1` must not beat a card's tuned value. Tiers 3-4 are caller instructions and sit on top. Gathering the tiers in the mixin rather than per provider is the fix for how the Anthropic, OpenAI-compat, and llama.cpp paths came to ignore `ModelSpec.generation_kwargs` outright. Provider-specific rewrites run *after* the merge and may still override the caller where an API demands it (Anthropic's forced `temperature=1` under extended thinking, the o-series rename). Whether a portable key survives at all is decided a step earlier, by `GENERATE_KWARG_SUPPORT` (above). **`client.default_generate_kwargs` is an input, not a report**: it starts empty (`{}`) on every provider via the base `__init__`, no provider overwrites it, and it is a delegating property on `ModelClient`/`AsyncModelClient`, `_AgenticView`, and `_AsyncInProcessClient` (so reassignment propagates to the inner client, not just mutation) and is pushed down `FallbackClient._load_state`. To read a card's profile, use `Model.generation_kwargs`. See [docs/how-to/control-thinking.md](docs/how-to/control-thinking.md) and [docs/explanation/thinking-and-context.md](docs/explanation/thinking-and-context.md) for the per-provider translation table and worked examples.

### Message Provenance (agent-loop vs user input)

Framework-injected turns are marked with a non-standard `"provenance"` key so a replayed or persisted transcript can distinguish them from genuine user input (both are `{"role": "user", ...}` and were otherwise byte-for-byte identical). The `Agent` loop tags its `final_answer_prompt` turn `PROVENANCE_FINAL_ANSWER`; it tags the `continuation_prompt` recovery nudge (see above) `PROVENANCE_CONTINUATION` when a turn comes back degenerate (no content and no tool calls); the personal-assistant example tags scheduler pushes `PROVENANCE_PROACTIVE`. Genuine user input and ordinary assistant turns are **left untagged** (absence = ordinary), so display logic is just `message.get(PROVENANCE_KEY)`. The constants and the key live in [aimu/models/_internal/message_meta.py](aimu/models/_internal/message_meta.py), re-exported from `aimu.models` and top-level `aimu` (`PROVENANCE_KEY`, `PROVENANCE_CONTINUATION`, `PROVENANCE_FINAL_ANSWER`, `PROVENANCE_PROACTIVE`). Tagging is done by index after the injecting `chat()` turn completes (the shared `_AgentLoopMixin._tag_injected_turn`, sync + async), mirroring how providers attach the `"thinking"` key by index, so the public `chat()` signature is untouched. `"provenance"` is in `INERT_MESSAGE_KEYS`, so `strip_inert_keys()` removes it (alongside `"thinking"` and the append-time `"timestamp"`) from every OpenAI-compat and Ollama request; Anthropic/HuggingFace drop it by rebuilding the payload. The Streamlit examples hide/mute tagged turns when re-rendering history. Streaming needs no marker: the injected prompts are *input*, never emitted as chunks, so the problem is purely at the replayed-history level. Tests: `tests/test_provenance.py` (+ async mirror in `tests/test_aio_agents.py`).

`supports_thinking` is the universal flag; *how* reasoning is requested is provider-specific and handled inside each client (Anthropic `thinking` param, HuggingFace `enable_thinking` template kwarg, `reasoning_effort` for OpenAI o-series). For OpenAI-compat and llama-cpp, reasoning arrives one of two ways and both are handled: a separate `reasoning_content` field on the delta/message (llama-server with the default `--reasoning-format deepseek`/`auto`, vLLM/SGLang reasoning parsers, which strip the tags server-side), **or** inline `<think>...</think>` tags in `content` (servers configured to pass tags through, e.g. `--reasoning-format none`). When `reasoning_content` is present it takes precedence and is not gated on `supports_thinking` (if the server sent it, it is reasoning); otherwise the `<think>` parser runs (`_ThinkingParser` streaming, `_split_thinking` non-streaming). Native Ollama uses its own `thinking` message field. The Anthropic client distinguishes two request shapes via a `ThinkingStyle` enum carried on each `AnthropicModel` member (analogous to HuggingFace's `ToolCallFormat`). `ThinkingStyle` stays Anthropic-internal: the cross-provider thinking-control capability (`thinking_levels`, `thinking_optional`) lives on `ModelSpec` for every provider, and `ThinkingStyle` only picks which of Anthropic's two request shapes a resolved level maps into:
- **`ThinkingStyle.ENABLED`**: `thinking={"type": "enabled", "budget_tokens": N}`; the model always thinks up to the budget. **Haiku 4.5 alone**, since the 4.6 line moved to `ADAPTIVE` (Anthropic deprecated the enabled+budget shape there and will remove it). A portable `thinking="low"/"medium"/"high"` maps to `budget_tokens` 2048/8000/16000 here; an explicit `thinking_budget_tokens` in `generate_kwargs` still wins over a level, and is warned about and dropped on every other member.
- **`ThinkingStyle.ADAPTIVE`**: `thinking={"type": "adaptive", "display": "summarized"}`; the model decides per request whether and how much to think (it may not think on simple prompts), and `temperature`/`top_p`/`top_k` are dropped. Required by Opus 4.7+, Sonnet 5 and Fable 5, which reject the `enabled` form with a 400, and used for Opus 4.6 / Sonnet 4.6, which accept it and where Anthropic deprecated the alternative. Turning thinking off on these models sends `{"type": "disabled"}` rather than omitting the parameter (Opus 5 and Sonnet 5 reason by default when it is absent); Fable 5 cannot be turned off at all. `display="summarized"` is set so reasoning still surfaces as `THINKING` chunks (the API default `"omitted"` yields empty thinking text). A `thinking=` **level** on these models becomes `output_config.effort` (`low`/`medium`/`high` -> `low`/`medium`/`xhigh`; `high` maps to `xhigh` because `high` is Anthropic's own default when the parameter is unset, so sending it would be a no-op -- the same reasoning as `QWEN_REASONING_EFFORT`). The vocabulary a model accepts is declared per member as `ModelSpec.effort_levels`, a tuple rather than a bool because `xhigh` exists on Opus 4.7+ but not on the 4.6 line (whose `high` therefore lands on `high`, its own top routine rung); an empty tuple means a level goes somewhere else (Haiku 4.5's `budget_tokens`) or nowhere. `_with_effort` **merges** into `output_config` rather than assigning, since it also carries `format` and a caller can already pass `output_config` through `generate_kwargs` (the only route to `max`) -- a caller's own `effort` is tier 4 and wins. `_cap_effort_for_disabled_thinking` lowers a caller's `xhigh`/`max` to `high` when thinking is disabled, warning: Opus 5 rejects that pair with a 400 and validates the two independently per request. Lowering the effort rather than reversing the disable is deliberate -- silently re-enabling reasoning the caller turned off is invisible except on the bill.
...
**Qwen 3.6 sampling correction**: the thinking-mode `presence_penalty` values Ollama and HuggingFace catalogued for Qwen 3.6 disagreed with the model cards, in opposite directions: Ollama used `0.9` and HuggingFace used `1.5`, while the 27B card specifies `0.0` (now corrected on both providers). The 35B-A3B MoE variant's card specifies a different value, `1.5`, so it keeps its own constant (`_QWEN_3_6_35B_THINKING_KWARGS`) rather than sharing the 27B's. Every Qwen 3.5/3.6/3.8 member across **every local-runtime catalog** now also carries a `nonthinking_generation_kwargs` instruct-mode profile (`_QWEN_INSTRUCT_KWARGS`), selected automatically when `thinking=False` resolves off (see `select_profile()` in `aimu/models/_internal/thinking.py`); HuggingFace still drops `presence_penalty` before generation (Transformers rejects it, so `HF_GENERATE_KWARGS` declares it `Unsupported`), leaving `temperature`/`top_p`/`top_k`/`min_p`/`repetition_penalty` as the values that actually reach generation on that path -- and of those five, only `temperature` and `top_p` hold different values in the two profiles (`top_k`, `min_p`, and `repetition_penalty` agree), so on HuggingFace switching modes is a two-parameter change. The profile now reaches the OpenAI-compat local server catalogs too (`OllamaOpenAIModel`, `LMStudioOpenAIModel`, `VLLMOpenAIModel`, `OMLXOpenAIModel`, etc.): since the `MODEL_FACTS` migration, they resolve their Qwen members' `nonthinking_generation_kwargs` from the same shared table Ollama and HuggingFace do, rather than declaring their own (they never did). So `thinking=False` now changes sampling on those paths as well, not only the template kwarg.
...
`omlx` is in `model_client._ADHOC_PROVIDERS` for a stronger reason than the other local servers:
...
- **[aimu/models/providers/ollama.py](aimu/models/providers/ollama.py)**: `OllamaEmbeddingClient` + `OllamaEmbeddingModel` (`nomic-embed-text`, `mxbai-embed-large`, `bge-m3`, `all-minilm`). Holds an `ollama.Client` (so it takes `host=` / `timeout=`) and calls `self._client.embed()`; pulls the model on construction (same as `OllamaClient`). Give it the same `host=` as the text client: embedding a corpus against one server and querying against another silently mixes vectors from two models.
...
### Turn Outcome (stop reason / truncation / refusal)

`client.last_stop_reason` holds the provider's own word for how the most recent turn ended (`"length"`, `"max_tokens"`, `"stop"`, `"refusal"`, ...), and `client.last_output_truncated` is the portable bool derived from it. **`_ChatStateMixin._record_stop_reason(stop_reason)` is the single seam**, and calling it is mandatory on every provider request path -- the same rule, for the same reason, as `_record_request`: `last_output_truncated` defaults to `False`, and that default means *"nobody looked"*, not *"not truncated"*. The agent loop turns it into an actionable `TruncatedTurnError` (`_ToolLoop._raise_if_truncated`, 8 call sites), so a provider that skips the seam does not merely lose a field -- it makes the error stop firing. Before v0.27.0 only Ollama set it, so on every other backend a turn cut off mid-answer surfaced as a bare empty string. `TRUNCATED_STOP_REASONS` (in `chat_state.py`) is the one vocabulary, since each backend spells it differently. Per provider: Anthropic reads `response.stop_reason` (via `_record_response`, which also does usage and the refusal check in one call, so a path cannot do two of three); OpenAI-compat reads `choices[0].finish_reason` through `_record_response` (complete responses) and `_record_stream_chunk` (streaming splits `finish_reason` and `usage` across different chunks, so each is recorded only when present); llama.cpp indexes the same field out of its plain dicts; HuggingFace has no field and infers it from generated length vs `max_new_tokens` (`_record_generated_length`, non-streamed only -- `TextIteratorStreamer` yields decoded text, so the streamed path leaves it `None`). Guarded by `tests/test_request_legibility.py::test_every_client_records_how_the_turn_ended`, parametrized over every installed client x chat/generate x stream/non-stream, with the shared stubs carrying a *non*-truncating value so it checks a real capture rather than a reset-to-`None`. Behaviour tests: `tests/test_stop_reason_api.py`.

**`ModelRefusalError`** (in `aimu/models/_base/shared.py`, exported from `aimu`, `aimu.models`, `aimu.aio`, alongside `ModelConnectionError` / `ContextOverflowError`) is raised when Anthropic's safety classifiers decline a request. That arrives as **HTTP 200 with `stop_reason: "refusal"` and no text block**, so every read path returned `""` and told the caller nothing; in an agent loop it read as a degenerate turn, so the continuation nudge fired and the run burned its iterations getting refused again. Opus 5 and Fable 5 ship the classifiers that produce it. `AnthropicClient._raise_if_refused` raises with the `stop_details` category and explanation attached (both may be absent), *after* usage is recorded so a caller can still see what the attempt cost. Being a distinct class, it composes with `FallbackClient(retry_on=(ModelRefusalError,))`, which is the vendor's own recommended recovery.
...
- **Ollama**: `timeout` only, forwarded to `ollama.Client(timeout=...)` / `ollama.AsyncClient(timeout=...)` (httpx). The **sync** `OllamaClient` holds an `ollama.Client` instance (`self._client`) and calls `self._client.chat/generate/pull` instead of the module-level `ollama.*` functions (the async client already used `ollama.AsyncClient`). Ollama has no native request-retry, so passing `max_retries` raises `ValueError` naming the `ollama-openai` provider (which routes through the OpenAI SDK and does support it). `OllamaEmbeddingClient` now holds a client instance too (see the `host=` note below), so it takes `timeout` as well.
...
**Remote Ollama (`host=`).** All three native Ollama clients (`OllamaClient`, `AsyncOllamaClient`, `OllamaEmbeddingClient`) take a `host` kwarg, keyword-only and forwarded verbatim to the ollama SDK (the same "use the SDK's own spelling, don't invent one" rule as `timeout`/`max_retries`). `_ollama_client_kwargs(host, timeout)` in [aimu/models/providers/ollama.py](aimu/models/providers/ollama.py) is the single place that builds the SDK constructor kwargs; the async twin imports it, matching the `OLLAMA_GENERATE_KWARGS` convention. It **omits** an unset value rather than passing `None`, so with no `host` the SDK's own resolution (`OLLAMA_HOST`, else `127.0.0.1:11434`) stays in charge, and it **raises** on a `/v1` suffix: the native API is served from the server root, so a copied OpenAI-compat endpoint would 404 at request time on `.../v1/api/chat`; the message points at the `ollama-openai` provider, which is the one that wants `/v1`. Two consequences worth knowing: `OllamaClient` pulls eagerly in `__init__`, so with `host=` the pull runs on the remote machine; and `OllamaEmbeddingClient` had to stop using module-level `ollama.pull`/`ollama.embed` for `host` to mean anything, which also fixes the silent mismatch where a remote text client paired with a localhost embedder. Discovery is deliberately **not** covered (see `available_text_models` above). Reaching the embedding client's real constructor params through `aimu.embedding_client()` required `ProviderEntry.direct_kwargs` (see the modality-factory note in Project Structure). Tests: `tests/test_ollama_host.py`.
...
- **`OpenAIClient`** (`aimu[openai_compat]`): thin subclass of `OpenAICompatClient` pointing at `https://api.openai.com/v1`; reads `OPENAI_API_KEY`. Overrides `_rewrite_generate_kwargs` to rename `max_tokens → max_completion_tokens` and force `temperature=1` for o-series models (o1/o3/o4 prefix). Lives in [aimu/models/providers/openai/text.py](aimu/models/providers/openai/text.py).
- **`AnthropicClient`** (`aimu[anthropic]`): full implementation using the native `anthropic` SDK (**1.x**, pinned `>=1,<2`: its HTTP layer is `httpx2`, and `temperature`/`top_p`/`top_k` are no longer keyword arguments of `messages.create()`); reads `ANTHROPIC_API_KEY`. `self.messages` is always stored in OpenAI format; conversion to Anthropic format (`_openai_messages_to_anthropic`) and tool format conversion (`_openai_tools_to_anthropic`) happen at call time. When the model calls tools, `_chat` stores the assistant message with Anthropic's **real** `tool_use` block IDs directly (via `_record_tool_calls`), so the engine's tool-result messages match them without a later id-patching pass. Thinking is native (not `<think>` tag parsing) and `_thinking_kwargs()` builds the request per the model's `ThinkingStyle`: `ENABLED` → `{"type": "enabled", "budget_tokens": N}` (Haiku 4.5 alone), disabled by omitting the parameter; `ADAPTIVE` → `{"type": "adaptive", "display": "summarized"}` (every member but Haiku 4.5; Opus 4.7+/Sonnet 5/Fable 5 400 on the `enabled` form, and the 4.6 line uses it because Anthropic deprecated theirs), disabled with an explicit `{"type": "disabled"}` since Opus 5 and Sonnet 5 reason when the parameter is *absent* (`_adaptive_thinking_kwargs` owns both adaptive branches and strips `temperature`/`top_p`/`top_k` on either, since those 400 on these models thinking or not). `CLAUDE_FABLE_5` declares `thinking_optional=False`: it always reasons and 400s on an explicit disable, so `thinking=False` warns at the resolver and never reaches the request. **Sampling parameters** are owned by one hook, `_route_sampling_kwargs` (called from `_rewrite_generate_kwargs`, so it runs on *every* request path -- including structured output, which routes around `_thinking_kwargs` and so cannot strip its own): all three are **dropped** when thinking is in effect or the model is in `_REJECTS_SAMPLING` (Opus 4.7+ and Sonnet 5 reject them outright), and otherwise **moved into `extra_body`**, the only route left in SDK 1.x. That membership set is deliberately *not* read off `ThinkingStyle`: the 4.6 line is adaptive **and** accepts sampling parameters, so the two are independent facts. Guarded by `tests/test_anthropic_sdk_contract.py`, which binds the payloads AIMU builds against the installed SDK's real `messages.create`/`stream` signature -- every other Anthropic test monkeypatches that method, so a removed keyword passes them all and fails only against the live API. Catalog: `CLAUDE_FABLE_5`, `CLAUDE_OPUS_5`, `CLAUDE_OPUS_4_8`, `CLAUDE_OPUS_4_7`, `CLAUDE_OPUS_4_6`, `CLAUDE_SONNET_5`, `CLAUDE_SONNET_4_6`, `CLAUDE_HAIKU_4_5`. See "Thinking Models" above for the `ThinkingStyle` rationale. **Opt-in prompt caching** (P1-A): the `cache_prompt: bool = False` constructor kwarg (Anthropic-only; threads via `aimu.client("anthropic:...", cache_prompt=True)`) makes the two format adapters mark the **system prompt** (returned as a `[{type:text, ..., cache_control:ephemeral}]` block list instead of a bare string) and the **last tool** (one breakpoint caches the whole tools array) with `cache_control: {"type":"ephemeral"}`. Because the markers live in the adapters, every request path is covered (chat, tool-follow-up, streaming, structured) with no per-call-site edits; the existing `system=system_str if system_str else NOT_GIVEN` guards still hold (a non-empty list is truthy). Below the provider minimum (1024 tokens; 2048 for Haiku) the API silently skips caching, so the flag is safe on. `usage_from_anthropic` adds `cache_creation_input_tokens`/`cache_read_input_tokens` to `last_usage` when present. Caching conversation *messages* is out of scope (they grow each turn). Tests: `tests/test_anthropic_caching.py`.
...
│   ├── context.py       # async summarize_messages only (count_tokens/trim_messages are pure)
...
│   │   ├── generate_kwargs.py # _GenerateKwargsMixin + merge_generate_kwargs/select_profile: kwarg tiers + provider rewrite hook
...
│   │   ├── factory.py       # ProviderEntry + build_client/resolve_model_string/FactoryDelegate: shared dispatch for the 5 modality factories (image/audio/speech/transcription/embedding); ProviderEntry.direct_kwargs names the params a client declares itself (forwarded as real kwargs), everything else is bundled into model_kwargs
...
├── events.py            # Run events: the RunEvent dataclass union, EventSink, emit(), log_events()
├── context.py           # count_tokens / trim_messages / summarize_messages (plain functions)
...
│   ├── validate.py      # validate_frontmatter(): the Agent Skills spec's own rules
│   ├── frontmatter.py   # split/load/render a SKILL.md's frontmatter; one definition of the format
│   ├── authoring.py     # write_skill/write_skill_script/update_skill + their three tool factories
