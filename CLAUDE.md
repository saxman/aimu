...
4. **Cross-provider consistency**: the `ModelSpec.id` is the provider-specific *wire* identifier (correctly different per provider: `qwen3:8b` vs `Qwen/Qwen3-8B` vs `qwen3-8b.gguf`), but the enum-member **name** (`QWEN_3_8B`) is the cross-provider identity `resolve_model_enum` searches for a bare name. When the same model ships under multiple providers, keep the name identical and keep the *intrinsic* capability flags (`tools`/`thinking`/`vision`) in agreement across providers. `tests/test_model_catalog_consistency.py` enforces this. `structured_output` and `audio` are excluded (serving-path flags: Ollama grammar-enforces JSON for any model; only some paths expose audio input). An intrinsic flag may still legitimately differ when a serving path can't deliver the capability (e.g. `GEMMA_3_12B` tools=False on in-process HF/Ollama, which lack a tool-parse format; `GEMMA_4_12B` vision=False on LlamaCpp, no default mmproj projector) -- set the flag to what the path can actually deliver, comment the rationale, and register it in the test's `_INTENTIONAL_DIVERGENCES`.
