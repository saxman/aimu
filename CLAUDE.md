...
**Qwen 3.6 sampling correction**: the thinking-mode `presence_penalty` values Ollama and HuggingFace catalogued for Qwen 3.6 disagreed with the model cards, in opposite directions: Ollama used `0.9` and HuggingFace used `1.5`, while the 27B card specifies `0.0` (now corrected on both providers). The 35B-A3B MoE variant's card specifies a different value, `1.5`, so it keeps its own constant (`_QWEN_3_6_35B_THINKING_KWARGS`) rather than sharing the 27B's. Every Qwen 3.5/3.6/3.8 member across Ollama, the OpenAI-compat local servers, and HuggingFace now also carries a `nonthinking_generation_kwargs` instruct-mode profile (`_QWEN_INSTRUCT_KWARGS`), selected automatically when `thinking=False` resolves off (see `select_profile()` in `aimu/models/_internal/thinking.py`); HuggingFace's `_update_generate_kwargs` still drops `presence_penalty` before generation (Transformers rejects it), so only `temperature`/`top_p`/`top_k`/`min_p` actually differ between modes on that path.
