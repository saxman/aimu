...
- **[aimu/tools/builtin.py](aimu/tools/builtin.py)**: built-in general-purpose tools, each decorated with `@tool` for direct in-process use. Pass an entire subgroup with `tools=builtin.web + [my_tool]`:
  ...
  - `get_web_content(url, max_chars=20000)`: Fetches a URL and returns Markdown, for an HTML page or a PDF. Classifies by `Content-Type` with `%PDF-` magic bytes as the tiebreaker, since a PDF served as `application/octet-stream` is common. HTML keeps its recovered `Published:` line; a PDF gets one `## Page N` heading per page, and is opened with an empty password first because published reports routinely carry an owner password alone. Any other `text/*` body comes back as-is; anything that is not text at all (an image, an archive) is refused, reported by media type and size rather than read as text. Two caps: `max_chars` on the return, whose marker names the parameter to raise, and a 10 MB module-level limit on the download, which refuses rather than truncates. Converters live in the private `aimu/tools/_documents.py`.
