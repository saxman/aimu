...
- **[One provider-agnostic client](https://saxman.github.io/aimu/how-to/switch-providers/).** A single interface covers Ollama, HuggingFace, llama-cpp, Claude, OpenAI, Gemini, and any OpenAI-compatible local server (LM Studio, vLLM, SGLang, llama-server, HF Transformers Serve, oMLX), and you swap providers with a `"provider:model_id"` string change.
- **[Reasoning, tools, and vision everywhere](https://saxman.github.io/aimu/reference/model-matrix/).** These capabilities work identically across every provider, and reasoning models surface their thinking as a distinct stream phase through the same API. Reasoning is also *steerable* with one portable parameter: `chat(..., thinking=False)` to skip it, or `thinking="low"` to dial the effort, translated per provider into Ollama's `think=`, `reasoning_effort`, or Anthropic's thinking budgets. A model that cannot honour the request warns and answers anyway, so the call site survives a model swap.
...
- **[Context management](https://saxman.github.io/aimu/how-to/manage-context/).** `aimu.trim_messages` and `aimu.summarize_messages` are plain functions over a conversation's `list[dict]`, never splitting a tool call from its result; `Agent(compaction=...)` runs one automatically before every turn and reports what it dropped through the events below.
...
- **[Tracing](https://saxman.github.io/aimu/how-to/stream-output/#trace-a-run).** See exactly what a run did: `extract_tool_calls(messages)` returns a clean list of `{iteration, tool, arguments, result}` records from any message history, and `pretty_print(stream)` renders a live run to the console with reasoning, tool calls, and output labelled. When you replay a stored history, the agent-loop prompts it injects (continuation and final-answer turns) carry a `provenance` marker so your UI can tell them apart from what the user actually typed.
- **[Run events](https://saxman.github.io/aimu/how-to/observe-a-run/).** Attach `events=` (a plain `Callable[[RunEvent], None]`) to a client, `Agent`, or workflow to see every model turn, tool call, and compaction as it happens; `aimu.events.log_events(logger)` is the one-line sink, and `client.last_request` exposes the exact payload sent after every kwarg merge and provider adaptation.
...
- **[Plain-function tools](https://saxman.github.io/aimu/how-to/add-custom-tool/).** Turn any plain function into a tool with `@tool`, where type hints and the docstring become the spec. The model's call arguments are validated and coerced against those hints before the tool runs, with a clear error returned to the model when they don't fit. Pass `tools=` to `chat()` or `Agent.run()` to override the tool set for one call (or `tools=[]` to disable).
...
- **[Pre-built tools](https://saxman.github.io/aimu/reference/api/tools/).** A library of ready-to-use tools ships in the box: web search and fetch, filesystem reads, a calculator, opt-in subprocess-isolated code execution and shell commands (isolation, not containment), timezone-aware date/time, and the generative-modality tools, grouped as `builtin.web` / `fs` / `compute` / `time` / `misc` / `image` / `audio` / `speech` / `transcription` to pass straight to `tools=`.
- **[MCP integration](https://saxman.github.io/aimu/how-to/use-mcp-tools/).** Bring cross-process FastMCP tools into the same registry with `MCPClient` and `mcp.as_tools()`, mixing them freely with `@tool` functions (`tools=builtin.web + mcp.as_tools()`). Connect a remote MCP service by URL with `MCPClient(url="https://...", auth="token")` (SSE / streamable-HTTP inferred).
- **[Agent skills](https://saxman.github.io/aimu/how-to/use-skills/).** A `SkillAgent` discovers `SKILL.md` files on the filesystem, implementing the [Agent Skills specification](https://agentskills.io/specification) (frontmatter validated on discovery, so a skill either loads as a valid skill or raises), and injects their instructions and tools on demand -- so you extend an agent by dropping in a folder instead of changing code. A skill's `scripts/` may declare their own dependencies inline with [PEP 723](https://peps.python.org/pep-0723/), which `uv` resolves per run, so a skill carries its own requirements rather than adding to yours. `SkillManager(include=[...])` scopes discovery when different agents should hold different skills.
...
- **[Document and conversation stores](https://saxman.github.io/aimu/how-to/use-document-memory/).** A path-keyed `DocumentStore` (in the spirit of the Claude memory tool, with AIMU's own method names) and a TinyDB-backed `ConversationManager` persist documents and chat history across sessions.
- **[Multi-user sessions](https://saxman.github.io/aimu/how-to/use-sessions/).** A `SessionStore` (in-memory or TinyDB) keeps per-conversation history keyed by `channel:sender`, so one process serves many users/chats; `SessionLocks` serializes a session's turns while different sessions run concurrently. Single-user collapses to a trivial default. The substrate for an always-on, multi-channel assistant.
