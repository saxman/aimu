...
The agent continues a tool-using run by calling `chat()` with no user message (a turn on the current messages), so it injects **no** synthetic user turn between successful tool rounds. Two narrower cases do inject a user turn: the opt-in `final_answer_prompt` at the iteration cap, and the `continuation_prompt` recovery nudge when a turn comes back degenerate (no content, no tool calls). A replayed history can't tell either from real input unless it's marked, so AIMU tags them with an inert `provenance` key (`message.get(aimu.PROVENANCE_KEY)` is `PROVENANCE_FINAL_ANSWER`, `PROVENANCE_CONTINUATION` for the recovery nudge, or `PROVENANCE_PROACTIVE` for scheduler pushes; genuine input is untagged). See [Persist conversations](persist-conversations.md#distinguish-agent-loop-turns-from-user-input) for the display pattern.
