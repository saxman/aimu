...
def approve(name, arguments):
    return name not in ("execute_python", "run_command")   # let sub-agents look things up, not run code or a shell unattended
