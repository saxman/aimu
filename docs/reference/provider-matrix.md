...
- **`HuggingFaceImageClient`** loads the `diffusers` pipeline lazily on the first `generate()` call. The spec's `pipeline_class` field names a class in the `diffusers` namespace (e.g. `"StableDiffusionXLPipeline"`, `"FluxPipeline"`). Placement: defaults to `device_map="balanced"` (sharding across GPUs) when >1 CUDA device is visible and `accelerate` is installed, else moves to CUDA/MPS. Override with `model_kwargs={"device": "cuda:1"}` or `model_kwargs={"device_map": ...}`. The audio and speech HF clients take the same `{"device": ...}` hint but have no sharding default (small models).
