...
The same inert-key mechanism carries a second annotation. Between successful tool-calling rounds the `Agent` loop continues by calling `chat()` with no user message, injecting nothing. It does inject a `{"role": "user", ...}` turn the human never typed in two narrower cases: the `final_answer_prompt` when the loop hits `max_iterations`, and the `continuation_prompt` recovery nudge when a turn comes back degenerate (no content, no tool calls). Scheduler "proactive" turns are similar (framework-initiated, not user-initiated). Left unmarked, the final-answer and proactive turns are byte-for-byte identical to real input, so a replayed or persisted transcript can't tell them apart.

AIMU tags them with a `"provenance"` key (`aimu.PROVENANCE_KEY`), whose value is one of `PROVENANCE_FINAL_ANSWER` or `PROVENANCE_PROACTIVE`. **Genuine user input, ordinary assistant turns, and the degenerate-turn recovery nudge are all left untagged** (absence means "ordinary turn"), so display logic is just `message.get(PROVENANCE_KEY)`. Like `"thinking"`, the key is inert (it is in `INERT_MESSAGE_KEYS`, so it is stripped before every provider request) and is set on the message by index after the injecting `chat()` turn completes, keeping the public `chat()` signature free of the concept. The Streamlit examples in `examples/web/` hide or mute tagged turns when re-rendering history.
