...
- *Smart context-detection* (same `client.chat()` routes by detecting a running loop). Return type becomes ambiguous (`str` vs coroutine); forgotten `await` silently returns a coroutine, violating [failures are apparent](design-principles.md#6-failures-are-apparent).
- *Async-first with hidden sync facade.* Sync users would carry an invisible event-loop thread and see asyncio frames in tracebacks, violating [plain Python](design-principles.md#1-plain-python).
