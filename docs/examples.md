...
| [personal-assistant/](https://github.com/saxman/aimu/tree/main/examples/personal-assistant) | An always-on, single-user assistant (OpenClaw / Hermes style) assembled from AIMU primitives: a `CLIChannel`, a `Scheduler` for proactive reminders, and a `SkillAgent` that authors its own skills. | [Build a personal assistant](how-to/build-personal-assistant.md) |
