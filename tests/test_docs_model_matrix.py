...
docs/reference/model-matrix.md's tables used to be maintained by hand, and drifted silently
twice: a new catalog member got no row, and a corrected capability flag got fixed in the enum
but not in the doc. The tables are now generated by scripts/generate_model_matrix.py (the prose
stays hand-written); test_matrix_tables_match_the_generator below checks the committed file
against the generator's output. The older tests in this file are kept as a second, independent
check straight against the catalogs, so a bug in the generator itself still fails the suite
instead of just reproducing whatever the generator got wrong.
"""
...
def test_matrix_tables_match_the_generator():
    """The tables are generated; the committed file must match what the generator emits.

    Prose sections stay hand-written. If this fails, run:
        python scripts/generate_model_matrix.py --write
    """
    import subprocess
    import sys

    result = subprocess.run(
        [sys.executable, str(paths.root / "scripts" / "generate_model_matrix.py")],
        capture_output=True,
        text=True,
        check=True,
    )
    assert result.stdout == MATRIX_PATH.read_text(), (
        "docs/reference/model-matrix.md is stale; regenerate with `python scripts/generate_model_matrix.py --write`"
    )
