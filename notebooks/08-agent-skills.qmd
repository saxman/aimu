...
- **stdin is closed.** Agents run in non-interactive shells, so a script that prompts gets `EOFError`
  immediately rather than blocking. Take input through flags, or through `env=` on
  `build_skills_server`, which a host uses to pass context a script cannot discover for itself.
