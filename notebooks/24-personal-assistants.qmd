...
> **Full access.** Skill scripts run with your user privileges, no sandbox (the OpenClaw/Hermes model). Only enable this for a model and inputs you trust. AIMU ships no sandbox at all -- `builtin.execute_python` included, which only stops accidents, not a deliberate attempt. For untrusted input, gate script authoring/execution behind `tool_approval`, or run the code in an external sandbox this library doesn't provide.
