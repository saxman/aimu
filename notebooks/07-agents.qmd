...
The `execute_python` built-in runs Python code in-process inside an agent -- useful for data analysis and computation. It is **not a security boundary** (restricted builtins and an import allowlist stop accidents, not attempts), so it is in `builtin.compute` but not `ALL_TOOLS`; opt in explicitly.
