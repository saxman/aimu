...
- **Change** `chat()` is now a **single model turn**, and the multi-turn tool-calling loop lives entirely in `Agent`. Previously each provider `_chat` executed a tool round **and** generated a follow-up answer, while `Agent.run()` *also* re-prompted with a `continuation_prompt` — so a tool-using turn produced the answer twice (the model answered, then answered again after the continuation prompt). Now: `chat()` issues one model request; if the model requests tools it executes them, appends the results, and returns (no follow-up generation). `chat()` gains an optional `user_message` (default `None` = "run a turn on the current messages, appending no new user turn" — the continuation primitive). `Agent` loops `chat(task)` then `chat()` until a turn makes no tool calls; **no synthetic continuation prompt is injected**. The stop condition (`_last_turn_called_tools`) now checks whether the latest model turn ended in tool calls rather than "any tool message since the last user turn". Applies to sync + `aimu.aio`, non-streaming + streaming, every provider, and `FallbackClient` (its `chat()` also takes the optional `user_message`). **Behavior change**: a bare `client.chat("q", tools=[...])` that triggers a tool now returns the tool turn (often empty) instead of a synthesized answer — use `Agent(client, tools=...).run("q")` (or `agent.as_model_client()`) for a full tool-using answer. **Deprecated (kept as accepted no-ops):** `Agent.continuation_prompt` / `DEFAULT_CONTINUATION_PROMPT`; `PROVENANCE_CONTINUATION` is no longer produced (kept for legacy transcripts). `max_iterations` and `final_answer_prompt` are unchanged.
