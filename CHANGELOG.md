...
- **New** the personal assistant can use **remote MCP servers**: a repeatable `--mcp URL` (with `--mcp-bearer TOKEN`) connects servers at startup, and an `add_mcp_server(url)` tool connects more mid-session (tools callable the same turn). Connect failures are logged and skipped (one bad server can't stop startup); a trust note covers that remote tools run whatever the service exposes.
- **New** the personal assistant has **persistent memory** (on by default; `--no-memory` to disable): a `SemanticMemoryStore` for facts about the user (`store_memory` / `search_memories`) and a `DocumentStore` for longer user-provided documents (`save_document` / `search_documents`), both persisted under the output dir so they survive restarts and span conversations. System-prompt guidance directs the model to store durable facts and save reference documents.
...
- **New** both personal-assistant channels can surface per-turn **reasoning and tool calls**, not just the final answer. `CLIChannel` gains opt-in `show_thinking` / `show_tools` flags (off by default, preserving the minimal library default); the example-local `WebChannel` emits `thinking` / `tool` frames the page renders as distinct blocks. The example enables both by default (`--no-show-thinking` / `--no-show-tools` to hide) via `AssistantConfig.show_thinking` / `show_tools`.
- **New** the personal assistant gets AIMU's **built-in tools** via a `--tools` flag selecting `builtin.*` groups (`web,fs,compute,misc` by default; `image`/`audio`/`speech`/`transcription` opt-in as they need an `AIMU_*_MODEL`; `all` / `none` accepted). Sync built-ins run on the async agent transparently via `asyncio.to_thread`.
