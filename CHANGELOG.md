...
- **New** **The public surface is pinned by test** (`tests/test_public_surface.py`). `import aimu`, `aimu.models`, `aimu.agents`, `aimu.tools`, and `aimu.aio` each get a captured `__all__` baseline, asserted as `REQUIRED ⊆ actual ⊆ REQUIRED ∪ CONDITIONAL` rather than set equality -- `aimu.models` builds its `__all__` from fifteen `HAS_*`-guarded blocks, and equality would bake one machine's installed extras into the test. A name entering or leaving the surface is now a deliberate edit to this file, not a side effect of some other change.
